...
\begin{itemize}
	...
	\vspace{.2cm} \\
	The operating system manages the resources offered by the computer hardware. It must also decide how to allocate these resources efficiently and fairly.
...
When a user program tries to run privileged commands, a trap occurs in which the OS evaluates the operating system evaluates the command. If it is accepted, then the mode bit switches to 0 and the command is executed in kernel mode. After which the mode bit id set again to 1. 

\subsubsection{Timer}
A \textbf{timer} can be set in order to interrupt the computer after a specific period of time. This period may be fixed or variable. If the timer throws an interrupt, then the control is passed to the operating system and an error message could be displayed.
...
To improve both the utilisation of CPU and the responsiveness of the computer, they must keep several programs in memory, creating a need for memory management. The OS is responsible for the following:
...
In order to make the computer system convenient for users, the OS provides a uniform, logical view of information storage. The operating system abstracts from the physical properties of its storage devices to define a logical storage unit, the \textbf{file}. The operating system maps files onto physical media and accesses these files via the storage devices. The OS is responsible for the following:
...
If a computer system has multiple users and allows concurrent execution of processes, then access to data must be regulated.
