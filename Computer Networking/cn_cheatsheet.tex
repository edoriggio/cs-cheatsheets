...
\begin{lcverbatim}
        ...
        +------------+           +-------------+
        |  Agent #1  | --SMTP--> |  Server #1  | -----+
        +------------+           +-------------+      |
                                                     SMTP
        +------------+    POP3   +-------------+      |
        |  Agent #2  | <--IMAP-- |  Server #2  | <----+
        +------------+    HTTP   +-------------+      
...
\begin{itemize}
	\item \textbf{POP3}
	\vspace{.2cm} \\
	The \textbf{POP3} (Postal Office Protocol) protocol allows a user to download emails from a server to its mail client. The emails can also be consulted while offline. Emails that are dowloaded are then removed from the server, thus cannot be accessed from multiple devices.
	\item \textbf{IMAP}
	\vspace{.2cm} \\
